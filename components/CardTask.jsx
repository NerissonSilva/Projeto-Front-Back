...
export function CardTask({ task, onDelete, onCheck, onFavorite, isEditing, editingText, onEditStart, onEditChange, onEditSave, onEditCancel }) {
  return (
    <div className="task">
      {isEditing ? (
        <div style={{ display: "flex", alignItems: "center", gap: "10px", flex: 1 }}>
          <input
            ...
            style={{ flex: 1, padding: "8px", fontSize: "14px" }}
          />
          <button onClick={onEditSave} style={{ background: "#4caf50" }}>Salvar</button>
          <button onClick={onEditCancel} style={{ background: "#999" }}>Cancelar</button>
        </div>
      ) : (
        <>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={() => onFavorite(task)} style={{ background: task.favorite ? "#f5c518" : "#ccc" }}>
              {task.favorite ? "⭐" : "☆"}
            </button>
            <button onClick={onEditStart} style={{ background: "#f0ad4e" }}>Editar</button>
            <button onClick={() => onDelete(task.objectId)} style={{ background: "#c0504d" }}>Remover</button>
          </div>
        </>
      )}
    </div>
